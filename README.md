...
The purpose of this project was to build fluency in the MERN stack, particularly in building and consuming RESTful APIs for CRUD functionality. I also wanted to work on authentication, animation, and web socket live chat features. The two obstacles I ran into ended up being:
...
These challenges inspired my next project, http://socketchat.xyz, which focused primarily on chat features and image uploads.
...
5. socketio chat connection string in client/src/pages/chat/Chat.js must be changed to the url that backend is running on, it is currently set to `` `?token=${token}` `` for production so it needs to be set to `` `http://localhost:8000?token=${token}` `` for use in a local environment
